...
function buildPromptForModel(modelId, req, roundsSoFar) {
    ...
    for (const round of roundsSoFar) {
        ...
        for (const resp of round.responses) {
            ...
            const shortReasoning = resp.reasoning.length > 140
                ? `${resp.reasoning.slice(0, 140)}...`
                : resp.reasoning;
            historyLines.push(`- ${resp.modelId} chose ${label} with reasoning: ${shortReasoning}`);
        }
    }
...
function parseChoiceFromText(rawText, numOptions) {
    ...
    if (choiceMatch) {
        ...
        if (idx >= 0 && idx < numOptions) {
            ...
            const reasoningQuoted = /\"reasoning\"\s*:\s*\"([\s\S]*?)\"\s*(?:,|\})/i.exec(trimmed);
            const reasoningQuotedToEnd = /\"reasoning\"\s*:\s*\"([\s\S]*)$/i.exec(trimmed);
            const reasoningUnquoted = /\"reasoning\"\s*:\s*([^,}\n\r]+)\s*(?:,|\})/i.exec(trimmed);
            let reasoning = '';
            const candidate = (reasoningQuoted && reasoningQuoted[1]) ||
...
async function runDebate(req) {
    ...
    for (let round = 1; round <= maxRounds; round++) {
        ...
        await Promise.all(models.map(async (modelId) => {
            ...
            try {
                const llmRes = await (0, llmClients_1.callModel)(modelId, prompt, {
                    temperature: req.temperature ?? 0.3,
                });
                const parsed = parseChoiceFromText(llmRes.text, req.options.length);
                const estimatedCost = (0, llmClients_1.estimateCostUsd)(modelId, llmRes.inputTokens, llmRes.outputTokens);
                roundResults.push({
                    modelId,
        ...
        const allAgree = firstChoice != null &&
    ...
    const finalReasoning = lastRound?.responses
    ...
    const result = {
        ...
        finalReasoning,
        consensus,
        totalEstimatedCostUsd: totalEstimatedCostUsd > 0 ? totalEstimatedCostUsd : undefined,
    };
    return result;
}
