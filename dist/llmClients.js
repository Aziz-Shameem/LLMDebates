...
async function callModel(modelId, prompt, options = {}) {
    switch (modelId) {
        case 'chatgpt':
...
async function resolveClaudeModelId() {
    if (process.env.ANTHROPIC_MODEL)
        return process.env.ANTHROPIC_MODEL;
    if (resolvedClaudeModelId)
...
async function resolveXaiGrokModelId() {
    ...
    const textOnlyIds = ids.filter((id) => !id.toLowerCase().includes('vision') &&
    ...
    const preferred = preference.find((p) => ids.includes(p)) ??
        textOnlyIds.find((id) => id.toLowerCase().includes('mini')) ??
        textOnlyIds.find((id) => id.toLowerCase().startsWith('grok-')) ??
        ids[0];
    if (!preferred)
        throw new Error('Could not resolve Grok model id');
...
async function callOpenAI(prompt, options) {
    if (!OPENAI_API_KEY) {
        ...
        return {
            text: '{"choice":"A","reasoning":"Mocked ChatGPT response (no OPENAI_API_KEY set)."}',
        };
    }
    ...
    const data = await res.json();
    const messageText = data.choices?.[0]?.message?.content ?? JSON.stringify(data);
...
async function callAnthropic(prompt, options) {
    if (!ANTHROPIC_API_KEY) {
        return {
            text: '{"choice":"A","reasoning":"Mocked Claude response (no ANTHROPIC_API_KEY set)."}',
        };
    }
    const temperature = options.temperature ?? 0.3;
    ...
    if (!res.ok) {
        ...
        const isModelNotFound = text.toLowerCase().includes('not_found') ||
...
async function callGemini(prompt, options) {
    if (!GEMINI_API_KEY) {
        return {
            text: '{"choice":"A","reasoning":"Mocked Gemini response (no GEMINI_API_KEY set)."}',
        };
    }
    const temperature = options.temperature ?? 0.3;
    ...
    const inputTokensRaw = usage.promptTokenCount ??
        usage.prompt_tokens ??
        usage.promptToken_count ??
        usage.promptTokenCount;
    const outputTokensRaw = usage.candidatesTokenCount ??
    ...
    return {
        ...
        inputTokens: typeof inputTokensRaw === 'number' ? inputTokensRaw : approxTokens(prompt),
        outputTokens: typeof outputTokensRaw === 'number'
            ? outputTokensRaw
            : approxTokens(text),
    };
}
async function callXaiGrok(prompt, options) {
    if (!XAI_API_KEY) {
        return {
            text: '{"choice":"A","reasoning":"Mocked Grok response (no XAI_API_KEY set)."}',
        };
    }
    const temperature = options.temperature ?? 0.3;
    ...
    if (!res.ok) {
        ...
        const isModelNotFound = text.toLowerCase().includes('model') &&
